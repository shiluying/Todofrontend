  ...
  kernel.fp.object = def(
    ...
    function () {
      var map = function (o, f) {
        var r = {}
        for (var i in o) {
          if (o.hasOwnProperty(i)) { r[i] = f(i, o[i]) }
        }
        return r
      }
     ...
     */
      var analyse = function (roots, modules) {
        ...
        var examine = function (name) {
          if (modules[name]) { children(name) } else { missing.push(name) }
        }
        ...
        var decycle = function (name) {
          if (array.contains(path, name)) { cycle = collect(path, name) } else { descend(name) }
        }
    ...
    function (ar, fn, map, piggybacker, stratifier) {
      var create = function (regulator, validator, onerror, define, require, demand) {
        ...
        var validate = function (onsuccess, results) {
          ...
          if (failed.length > 0) { onerror('Fetcher error: modules were not defined: ' + failed.join(', ')) } else { onsuccess() }
        }
  ...
  kernel.module.loader = def(
    ...
    function (analyser) {
      var load = function (roots, deps, fetcher, oncontinue, onsuccess, onerror) {
        ...
        if (result.cycle) { onerror('Dependency error: a circular module dependency exists from ' + result.cycle.join(' ~> ')) } else if (result.load.length === 0) { onsuccess() } else { fetcher.fetch(result.load, oncontinue) }
      }
    ...
    function (ar, obj, loader, fetcher) {
      var create = function (regulator, onerror) {
        ...
        var define = function (id, dependencies, definition) {
          if (id === undefined) { onerror('Define error: module id can not be undefined') } else if (blueprints[id] !== undefined) { onerror("Define error: module '" + id + "' is already defined") } else { blueprints[id] = { id: id, dependencies: dependencies, definition: definition } }
        }
        ...
        var demand = function (id) {
          ...
          if (result === undefined) { throw "module '" + id + "' returned undefined from definition function" }
          modules[id] = result
          return result
        }
    ...
    function (ar, fn) {
       ...
       */
        var regulate = function (ids, define, require, demand, onsuccess, onerror) {
          ...
          if (unresolved.length === 0) { resolve(ids, define, require, demand, onsuccess, onerror) } else { delay(unresolved, ids, define, require, demand, onsuccess, onerror) }
        }
  ...
  loader.tag.script = def(
    ...
    function () {
      var guard = function (callback) {
        return function (evt) {
          if (evt.srcElement.readyState === 'loaded' || evt.srcElement.readyState === 'complete') { callback() }
        }
      }
      ...
      var onload = function (el, callback) {
        if (ie(el)) { el.attachEvent('onreadystatechange', guard(callback)) } else { el.addEventListener('load', callback, false) }
      }
        ...
        fs.exists(url, function (exists) {
          if (exists) {
            fs.readFile(url, 'UTF-8', function (err, data) {
              if (err) { error('Error reading file [' + url + '], error [' + err + ']') } else { success(data) }
            })
  ...
  loader.transporter.xhr = def(
    ...
    function () {
      ...
      var done = function (req, url, success, error) {
        if (req.status === 200 || req.status === 304) { success(req.responseText) } else { error('Transport error: ' + req.status + ' ' + req.statusText + ' for resource: "' + url + '"') }
      }
      ...
      var request = function (url, success, error) {
        ...
        if (req) { getUrl(req, url, success, error) } else { error('Transport error: browser does not support XMLHttpRequest.') }
      }
  ...
  module.reader.bouncing = def(
    ...
    function (ar, error, specs) {
      ...
      var tick = function (file, cfg, done, read, acc) {
        ...
        if (accumulated.configs.length > 0) { bounce(done, read, accumulated) } else { done({ sources: accumulated.sources, types: accumulated.types }) }
      }
